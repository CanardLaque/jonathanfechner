...
myApp.controller('MainController', ['$scope', 'Topic', '$routeParams', function($scope, Topic, $routeParams) {

    $scope.topic = Topic;
    $scope.selectedTopic = Topic[0].list[0];

    $scope.selectTopic = function(thetopic) {
        $scope.selectedTopic = thetopic;
    };


}]);

myApp.service('Topic', function() {
    return [
        {
            type: 'rouge',
            list: [{
